...
默认配置下，监听器 TmkTranslationListener 的回调在主线程执行。通道配置 `setTTSAudioCallbackThread(.background)` 可将 TTS PCM 的 `onAudioDataReceive` 切到 SDK 管理的串行后台队列；识别、翻译、状态、事件和错误回调仍在主线程，listener 方法签名和原有接入流程不变。后台音频回调中需要更新 UI 时，请切回主线程。

该配置适用于在线和离线通道的 TTS PCM 回调。
...
- 设置自定义离线模型仓库根地址；传 nil、空字符串或非法地址时沿用当前环境原有默认下载源。

- 支持带端口和多级路径的 HTTP/HTTPS 地址，例如 `http://192.168.0.56:9999/tmk-models/v3.3/`；不能包含用户信息、query 或 fragment。

- 模型包路径按 `asr/{lang}.zip`、`mt/{src}2{dst}.zip`、`tts/{lang}.zip` 拼接。
...
#### setTTSAudioCallbackThread\(\_:\)

设置 TTS PCM 的 `onAudioDataReceive` 回调线程，参数类型为 `TmkTTSAudioCallbackThread`。不调用时保持 `.main`。

|取值|行为|
|---|---|
|`.main`|默认值，在主线程回调 PCM|
|`.background`|由 SDK 为当前通道创建串行后台队列，按提交顺序回调 PCM|

在原有通道 Builder 上设置该选项，其他配置和 listener 接入保持不变：

```swift
let channelConfigBuilder = TmkTranslationChannelConfig.Builder()
    .setTTSAudioCallbackThread(.background)
```

该配置适用于在线和离线通道，只影响 TTS PCM 回调，不改变识别、翻译、状态、事件和错误回调线程。后台 PCM 与主线程回调可能并发，共享状态应做好同步，UI 更新须切回主线程；无需传入或管理 SDK 的后台队列。
...
切换 `setOfflineModelBaseURL` 后，当前语言对所需包会进入 `needsUpdate` 并重新下载。新包先解压到临时目录，经过响应长度（服务端提供时）、ZIP 条目大小/CRC、Zip Slip 和模型关键文件校验后，才逐包替换旧目录；失败时旧模型目录不被覆盖。当前服务端不提供 SHA-256，因此这些检查不能证明下载源真实性。使用内网 HTTP 地址时，宿主 App 需配置相应 ATS 策略，正式环境建议使用 HTTPS。
...
- 默认 `.main` 时，TmkTranslationListener 的所有回调均在主线程执行。
- 设置 `setTTSAudioCallbackThread(.background)` 后，在线和离线通道仅 TTS PCM 的 `onAudioDataReceive` 在 SDK 串行后台队列执行；其他 listener 回调仍在主线程。后台回调中更新 UI 须切回主线程。
...
- 只有在 setDiagnosisConfig\(TmkDiagnosisConfig\(enabled: true\)\) 后，SDK 才会产生诊断文件。
...
诊断日志默认加密落盘。未配置集成方公钥时，密文仅能由 Timekettle 解密；配置公钥后，同一个文件会同时包含 Timekettle 和集成方两个接收方的密钥信封，集成方可使用自己的私钥离线解密。

```Swift
let hostPublicKeyPem = """
-----BEGIN PUBLIC KEY-----
...
let globalConfig = TmkTranslationGlobalConfig.Builder()
    .setAuth(appId: "your_app_id", secret: "your_app_secret")
    .setDiagnosisConfig(.init(enabled: true))
    .setDiagnosisEncryptionEnabled(true)
    .setDiagnosisPublicKey(hostPublicKeyPem)
    .build()

TmkTranslationSDK.shared.sdkInit(globalConfig)
```

- `setDiagnosisPublicKey(_:)` 支持 P\-256 PEM\(SPKI\) 或 base64 X9\.63 公钥。私钥必须保存在集成方的密钥管理系统中，不得打包进 App、提交到仓库或随诊断包传输。

- 非法或非 P\-256 公钥不会中断 SDK，也不会退化为明文；SDK 会记录警告并仅保留 Timekettle 接收方。

- 可使用 `isDiagnosisEncryptionEnabled()` 确认加密是否生效，使用 `getDiagnosisRecipientKids()` 获取当前接收方 kid。

- 加密文件扩展名为 `.tlog`，诊断音频扩展名为 `.tpcm`。`getDiagnosisDirectoryURL()` 会先刷新当前加密缓冲，再返回可供业务侧打包和分享的目录。

安装 Python 依赖后，使用仓库中的统一解密工具：

```Shell
python3 -m pip install cryptography
python3 tools/diagnosis/decrypt_diagnosis.py \
    --key /secure/host_private.pem \
    --input sdk_diagnosis.zip \
    --output ./decrypted
```

`--input` 也支持诊断目录、单个 `.tlog` 或 `.tpcm` 文件。工具会将文本恢复为 `.log`，将音频恢复为 `.pcm`；受密码保护的 PEM 私钥可额外传入 `--password`。

#### 4 KB 分块与退出行为

- 4 KB 是文本日志单个加密块的最大明文长度，不要求日志凑满 4 KB。正常刷新、释放通道、销毁 SDK 或读取诊断目录时，不足 4 KB 的尾部也会立即封装成独立的有效密文块；警告和错误日志会立即刷新。App 进入后台或正常终止时，SDK 也会尽力刷新。

- 已完成的块可独立认证和解密。进程被系统直接终止、崩溃或设备掉电时，已经写入的块不会因为最后一块不完整而整体解密失败。

- 异常终止发生在刷新前时，当前不足 4 KB 的文本尾块仍在内存中，可能丢失；启用音频诊断时，每个活跃音频文件对应的未刷新块上限为 32 KB。上层异步队列中尚未交给加密写入器的数据也无法保证保留，因此异常退出时的总丢失量不能严格限定为一个块。

- 业务侧分享或上传前应通过 `getDiagnosisDirectoryURL()` 获取目录，避免直接读取正在写入的文件。生产环境不建议通过关闭加密规避尾块风险。
...
- 释放在线、离线两个翻译运行时及其会话相关资源。

- 会停止并释放在线、离线两个运行时的通道引擎，取消两类未完成建房请求，异步关闭已创建的在线房间，并清理离线模型下载管理器等会话状态。

- 不会清空 sdkInit 的全局配置，也不会清空鉴权状态。

- 同一个 SDK 实例可同时保留一个在线和一个离线运行时；重复创建同类通道会替换旧通道。若只结束其中一路，请持有对应 `TmkTranslationChannel` 并调用 `release()`，不要调用 `releaseChannel()`。
...
- releaseChannel\(\)：释放在线、离线两个翻译运行时及其会话相关资源，包含两类通道、未完成建房请求、在线房间关闭和离线模型下载管理器等会话状态。整体退出或重置两路运行时时调用；单路结束使用对应 `TmkTranslationChannel.release()`。
...
同一个 SDK 实例可同时保留一个在线和一个离线运行时；每类运行时仅保留一个通道，重复创建同类通道会替换旧通道。整体释放时调用 TmkTranslationSDK\.shared\.releaseChannel\(\)；只切换其中一路时释放对应通道，避免影响另一运行时。
